...
/** Canonicalize Accept header to something Chronicle understands
 *
...
/*
 * bound to response object.
 * Not sure I am going to need this one - not in use yet.
 */
module.exports.stream_ok = function (request, in_stream) {
    var accept = determine_accept( request.headers.accept );
    ...
    // this is taking from stdin - because we were passed data
    // we need two types of transforms - one from file and one from stream
    args = [ transform, '-' ];
    spawn( 'xsltproc', args );

    // Check request to see if we need to modify response?
...
/*
 * THIS IS NOT IN USE YET - and may not ever be
 */
function stream_response(request, code, data, input_headers) {
    var headers = {
        'Access-Control-Allow-Origin': '*',
        'Content-Type': content_type
    };
